...
def oxygen_mixed_functionalization(gen, r_angstrom, bond_tol=0.2,
    ...
    def too_close(new_xyz, sym, existing):
        for s, x, y, z in existing:
            ...
            if d < 0.5:
                return True  # absolute guardrail
            # pairwise distance thresholds
    ...
    for idx in chosen_OH:
        pos = coords[idx]
        rvec = pos / np.linalg.norm(pos)              # C–O direction
        O_pos = pos + BOND_CO * rvec

        # --- construct O–H direction with correct 104.5° C–O–H angle ---
        ...
        rot_dir = np.cos(theta) * (-rvec) + np.sin(theta) * (np.cos(phi) * x + np.sin(phi) * y)
        rot_dir /= np.linalg.norm(rot_dir)
        H_pos = O_pos + BOND_OH * rot_dir

        # --- skip placement if either atom overlaps any existing atom ---
        if too_close(O_pos, "O", existing) or too_close(H_pos, "H", existing):
            continue
